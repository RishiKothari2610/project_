...
struct node *root=NULL;  //initialized root pointer
struct node *create_node(int);  // function prototyping
void insert(int);
...
void insert(int data)  // inserts the data in the BST
{
    ...
    if(new_node!=NULL)
    {
        if(root==NULL)  // if the root is empty then make a new node as the root node
        {
            root=new_node;
        ...
        while(temp!=NULL)  // traverse through the BST to get the correct position for insertion
        {
            prev=temp;
            if(data>temp->data)
            {
        ...
        if(data>prev->data)  // found the last node where the new node should insert
        {
            prev->right=new_node;
        }
        else
...
int search(int key)  // search the given key node in BST
{
    struct node *temp=root;
    while(temp!=NULL)
    {
...
struct node *smallest_node(struct node *root)  // finds the node with the smallest value in BST
{
...
int get_data()   // getting data from the user
{
    int data;
    printf("\nEnter Data: ");
    scanf("%d",&data);
